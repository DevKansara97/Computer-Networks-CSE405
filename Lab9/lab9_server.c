...
int main() {
    ...
    struct timeval tv = {1, 50}; // 1 sec, 50 u_sec
    ...
    while (1) {
        ...
        while (next_seq < base + WINDOW_SIZE) { // packet in order
            ...
            int data_len = fread(buffer, 1, PACKET_SIZE, fp);
            if (data_len <= 0) break;
            ...
            *(uint16_t *)&buffer[idx] = htons(data_len);
            ...
            memcpy(&buffer[idx], buffer, data_len);
